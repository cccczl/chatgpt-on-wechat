    ...
    for m in msgList:
        ...
        if m['MsgType'] == 1: # words
            if m['Url']:
                ...
                data = 'Map' if data is None else data[1]
                msg = {
                    'Type': 'Map',
                    'Text': data,}
            else:
            ...
            regx = r'\[CDATA\[(.+?)\]\]'
            data = re.search(regx, m['Content'])
            data = 'System message' if data is None else data[1].replace('\\', '')
            msg = {
                'Type': 'Note',
                'Text': data, }
...
async def send_msg(self, msg='Test Message', toUserName=None):
    ...
    return await self.send_raw_msg(1, msg, toUserName)
