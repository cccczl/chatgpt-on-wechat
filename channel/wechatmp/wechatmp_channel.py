...
class WechatMPChannel(ChatChannel):
    ...
    async def delete_media(self, media_id):
        logger.debug(f"[wechatmp] permanent media {media_id} will be deleted in 10s")
        await asyncio.sleep(10)
        self.client.material.delete(media_id)
        logger.info(f"[wechatmp] permanent media {media_id} has been deleted")
    ...
    def send(self, reply: Reply, context: Context):
        ...
        if self.passive_reply:
            if reply.type in [ReplyType.TEXT, ReplyType.INFO, ReplyType.ERROR]:
                ...
                logger.info(f"[wechatmp] text cached, receiver {receiver}\n{reply_text}")
                ...
                try:
                    ...
                    with open(voice_file_path, "rb") as f:
                        ...
                        logger.debug(f"[wechatmp] upload voice response: {response}")
                        # 根据文件大小估计一个微信自动审核的时间，审核结束前返回将会导致语音无法播放，这个估计有待验证
                        f_size = os.fstat(f.fileno()).st_size
                        time.sleep(1.0 + 2 * f_size / 1024 / 1024)
                                        # todo check media_id
                except WeChatClientException as e:
                    logger.error(f"[wechatmp] upload voice failed: {e}")
                    return
                media_id = response["media_id"]
                logger.info(
                    f"[wechatmp] voice uploaded, receiver {receiver}, media_id {media_id}"
                )
                ...
                filename = f"{receiver}-" + str(context["msg"].msg_id) + "." + image_type
                ...
                try:
                    ...
                    logger.error(f"[wechatmp] upload image failed: {e}")
                    return
                media_id = response["media_id"]
                logger.info(
                    f"[wechatmp] image uploaded, receiver {receiver}, media_id {media_id}"
                )
                ...
                self.cache_dict[receiver] = ("image", media_id)
        elif reply.type in [ReplyType.TEXT, ReplyType.INFO, ReplyType.ERROR]:
            reply_text = reply.content
            texts = split_string_by_utf8_length(reply_text, MAX_UTF8_LEN)
            if len(texts) > 1:
                logger.info(f"[wechatmp] text too long, split into {len(texts)} parts")
            for i, text in enumerate(texts):
                self.client.message.send_text(receiver, text)
                if i != len(texts) - 1:
                    time.sleep(0.5)  # 休眠0.5秒，防止发送过快乱序
            logger.info(f"[wechatmp] Do send text to {receiver}: {reply_text}")
        elif reply.type == ReplyType.VOICE:
            try:
                ...
                if file_type == ".mp3":
                    ...
                    mp3_file = f"{os.path.splitext(file_path)[0]}.mp3"
                    any_to_mp3(file_path, mp3_file)
                    file_path = mp3_file
                    file_name = os.path.basename(file_path)
                    file_type = "audio/mpeg"
                logger.info(f"[wechatmp] file_name: {file_name}, file_type: {file_type} ")
            ...
            logger.info(f"[wechatmp] Do send voice to {receiver}")
        elif reply.type == ReplyType.IMAGE_URL:  # 从网络下载图片
            img_url = reply.content
            pic_res = requests.get(img_url, stream=True)
            image_storage = io.BytesIO()
            ...
            logger.info(f"[wechatmp] Do send image to {receiver}")
        elif reply.type == ReplyType.IMAGE:  # 从文件读取图片
            image_storage = reply.content
            image_storage.seek(0)
            image_type = imghdr.what(image_storage)
    ...
    def _success_callback(self, session_id, context, **kwargs):  # 线程异常结束时的回调函数
        logger.debug(
            f'[wechatmp] Success to generate reply, msgId={context["msg"].msg_id}'
        )
        if self.passive_reply:
    ...
    def _fail_callback(self, session_id, exception, context, **kwargs):  # 线程异常结束时的回调函数
        logger.exception(
            f'[wechatmp] Fail to generate reply to user, msgId={context["msg"].msg_id}, exception={exception}'
        )
        if self.passive_reply:
